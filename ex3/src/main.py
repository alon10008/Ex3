...
def main():
    ga = GraphAlgo()
    files = ["data/G_10_80_1.json", "data/G_100_800_1.json", "data/G_1000_8000_1.json", "data/G_10000_80000_1.json", "data/G_20000_160000_1.json", "data/G_30000_240000_1.json"]
    for f in files:
        netx = []
        digra = []
        g = init_networkx(f)
        ...
        netx.append((end-start))
        start = int(round(time.time() * 1000))
        ga.connected_components()
        end = int(round(time.time() * 1000))
        digra.append((end-start))
        ...
        start = int(round(time.time() * 1000))
        nx.dijkstra_path(g, 0, ga.g.v_size()-1)
        end = int(round(time.time() * 1000))
        netx.append((end - start))
        start = int(round(time.time() * 1000))
        ga.shortest_path(0, ga.g.v_size()-1)
        end = int(round(time.time() * 1000))
        digra.append((end - start))
        print(f)
        print("networkx:\nconnected_components: " + str(netx[0]) + "[millisec]\nshortest_path: " + str(netx[1]) + "[millisec]")
        print("GraphAlgo:\nconnected_components: " + str(digra[0]) + "[millisec]")
        print("connected_component: " + str(digra[1]) + "[millisec]")
        print("shortest_path: " + str(digra[2]) + "[millisec]\n")


def init_networkx(file_j):
    g = nx.DiGraph()
    with open(file_j) as f:
        j = json.load(f)
        for n in j["Nodes"]:
            g.add_node(n["id"])
        for e in j["Edges"]:
